...
st.markdown("<style>.synthetic{background:#7b1f1f;color:white;padding:.55rem;text-align:center;font-weight:700}.stMetric{border:1px solid #d4d8dd;padding:.7rem;border-radius:.4rem}</style>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.button("Start new exercise", type="primary"):
        st.session_state.update(started=True, phase=0, decisions=[], questions=[], start=datetime.now(timezone.utc).isoformat())
    st.session_state.participants = st.multiselect("Participants", ROLES, default=st.session_state.participants)
...
with tabs[0]:
    ...
    for idx, inject in enumerate(injects[:reveal], 1): st.info(f"Inject {phase}.{idx} | {inject} | Synthetic evidence")
    st.text_area("Facilitator evidence and observations", key=f"evidence_{phase}")
    question = st.text_input("Unresolved question")
    if st.button("Track question") and question: st.session_state.questions.append(question)
with tabs[1]:
    ...
    for service in services:
        if service.name in selected:
            ...
            rows.append({"Service": service.name, "MTD": service.tolerance.mtd_minutes, "Consumed": f"{result['consumed_pct']}%", "Status": result["rag"]})
    st.dataframe(rows, use_container_width=True, hide_index=True)
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Tier 0 at risk", sum(1 for x in candidates if x.approved)); c2.metric("Payment status", "Decision required" if selected else "Within baseline")
...
with tabs[3]:
    with st.form("decision"):
        ...
        assumptions = st.text_area("Assumptions / unknowns / evidence needed"); dissent = st.text_area("Dissent")
...
with tabs[4]:
    ...
    if st.button("Generate board PDF"):
        target = Path("exercise-output/board-packet.pdf"); write_pdf(board, target)
        st.download_button("Download board packet (PDF)", target.read_bytes(), target.name, "application/pdf")
    st.caption("Reports preserve empty findings until exercise evidence is entered; scoring and conclusions remain reviewable.")
